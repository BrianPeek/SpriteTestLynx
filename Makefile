...
#EMUCMD := c:\mame\mame64 lynx -cfg_directory c:\mame\cfg -video gdi -nomax -window -rp c:\mame\roms -skip_gameinfo -cart
EMUCMD := "C:\Users\brian\OneDrive\Projects\Lynx\Handy-0.95-1\handy.exe"
...
SOURCES += $(wildcard $(SRCDIR)/font/*.pcx)
SOURCES += $(wildcard $(SRCDIR)/gfx/*.pcx)
SOURCES += $(wildcard $(SRCDIR)/sfx/*.s)

# Add to SOURCES something like 'src/c64/me.c src/c64/too.s'.
# Use of assembler files with names ending differently than .s is deprecated!
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/*.c)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/title/*.c)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/title/gfx/*.pcx)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/game/*.c)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/game/gfx/*.pcx)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/*.s)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/sfx/*.s)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/gfx/*.pcx)
# SOURCES += $(wildcard $(SRCDIR)/$(TARGETLIST)/gfx/font/*.pcx)
...
##########################
## Core (default segment)
vpath %.c $(SRCDIR)/$(TARGETLIST) $(SRCDIR)
$(TARGETOBJDIR)/%.o: %.c | $(TARGETOBJDIR)
	$(CC) -t $(CC65TARGET) -c --create-dep $(@:.o=.d) $(CFLAGS) -o $@ $<

vpath %.s $(SRCDIR)/$(TARGETLIST) $(SRCDIR) $(SRCDIR)
$(TARGETOBJDIR)/%.o: %.s | $(TARGETOBJDIR)
	$(CC) -t $(CC65TARGET) -c --create-dep $(@:.o=.d) $(ASFLAGS) -o $@ $<

vpath %.s $(SRCDIR)/$(TARGETLIST) $(SRCDIR) $(SRCDIR)/sfx
$(TARGETOBJDIR)/%.o: core/sfx/%.s | $(TARGETOBJDIR)
	$(CC) -t $(CC65TARGET) -c --create-dep $(@:.o=.d) $(ASFLAGS) -o $@ $<

vpath %.pcx $(SRCDIR)/$(TARGETLIST) $(SRCDIR) $(SRCDIR)/gfx
$(TARGETOBJDIR)/%.o: core/gfx/%.pcx | $(TARGETOBJDIR)
	$(SP) -r $< -c lynx-sprite,mode=packed,ax=0,ay=0 -w converted\$*.c,ident=$*
	$(CC) -t $(CC65TARGET) -c $(CFLAGS) -o $@ converted\$*.c

##########################

##########################
## Font
vpath %.pcx $(SRCDIR)/$(TARGETLIST) $(SRCDIR) $(SRCDIR)/font
$(TARGETOBJDIR)/%.o: core/font/%.pcx | $(TARGETOBJDIR)
	$(SP) -r $< -c lynx-sprite,mode=packed,ax=0,ay=0 -w converted\$*.c,ident=$*
	$(CC) -t $(CC65TARGET) -c $(CFLAGS) -o $@ converted\$*.c
##########################
...
## Standard graphics
vpath %.pcx $(SRCDIR)/$(TARGETLIST) $(SRCDIR) $(SRCDIR)/gfx
$(TARGETOBJDIR)/%.o: %.pcx | $(TARGETOBJDIR)
	$(SP) -r $< -c lynx-sprite,mode=packed,ax=0,ay=0 -w converted\$*.c,ident=$*
	$(CC) -t $(CC65TARGET) -c $(CFLAGS) -o $@ converted\$*.c
##########################
